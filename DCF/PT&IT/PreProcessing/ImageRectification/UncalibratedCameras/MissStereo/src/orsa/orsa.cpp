/* ORSA (Optimized RANSAC): filter out outlier matches.
    ...
    Copyright (C) 2010 Zhongwei Tang <tangfrch@gmail.com>
...
/// ORSA algorithm for robust fundamental matrix computation.
/// Filter out outliers among matches in @match and output @inliers. @t is the
/// number of samples to draw, @verb is a flag for verbose mode (0 means no 
/// display). @stop is a flag to indicate if the algorithm should stop as soon
/// as a meaningful sample is found. @logalpha0 is the log probability of having
/// an error of 1 pixel. @errorMax is the computed threshold for discrimation
/// inlier/outlier.
/// @mode is an integer with the following meaning:
     ...
     std::vector<size_t>& inliers, float& errorMax)
{
    ...
    size_t n = match.size();
    if (n <= 7) {
        std::cerr << "orsa:: error, more than 7 correspondences are needed."
                  << std::endl;
