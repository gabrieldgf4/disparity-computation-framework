% Multi-resolution and Perceptual Grouping Algorithm
...
function [D, all_maps] = mrpg(I1, I2, min_d, max_d, method, h, w, reverse, ...
    pyr_levels, gamma_s, kernel, Tmax, dim_x, dim_y)
...
for i=p_h+1:height-p_h
    for j=p_w+1:width-p_w
        ...
        for k = ini_disp : end_disp-1 
            displacement = double(j+(-k*reverse)); % to be used in reverse or not reverse
            if displacement-p_w < 1 || displacement+p_w > width
                ...
                win_C = C(i-p_h:i+p_h, j-p_w:j+p_w, k+1);
                ...
                % Equation (2), color similarity grouping
                color_group_I2 = exp(-diff_from_center_p_I2./gamma_s);
                
                % Equation (6), support weight window
                W_I2 = (color_group_I2 .* proximity);
        
                % Equation (8), The Dissimilarity Cost
                E(i,j,k+1) = sum(nansum(win_C .* W_I1 .* W_I2)) / sum(nansum(W_I1 .* W_I2));
            end
