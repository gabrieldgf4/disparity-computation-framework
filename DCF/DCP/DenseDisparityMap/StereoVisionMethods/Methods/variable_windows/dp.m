...
function M_out = dp(Cost, Mx_out, My_out, Used_Windows)
       ...
       for x=2:heigth+1  
        ...
        for y=2:width+1   
            ...
            elseif Mx(x-1,y) < x && My(x,y-1) >= y
               ...
               x1 =  x - h_max + 1; % x1 = x -k +1;
               y1 =  y - h_max + 1; % y1 = y -k +1;
               ...
               if y1 <= 1
                ...
                for linha_up=x:-1:x1
                    ...
                    %max_w_lin = max(Used_Windows_Disp(linha_up,2:y));
                    %y1 = y - max_w_lin + 1; % y1 = y -k +1;
                    ...
                    for regressiva=y:-1:y1
                        ...
                        if (Mx_origen(linha_up,regressiva) < x) || (My_origen(linha_up,regressiva) < y)
                            continue;
                        end
                        
                        [c_min, index] = min([C_W(linha_up,regressiva), c_min_marcador(1,1,1)]);
                            if index==1
                                c_min_marcador(1,1,:) = [c_min, Mx_origen(linha_up,regressiva), My_origen(linha_up,regressiva)];
                            end
